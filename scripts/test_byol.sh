...
#SBATCH --gres=dcu:4
#SBATCH --mem=100G
#SBATCH -o Test_BYOL.o
#SBATCH -e Test_BYOL.e
...
rm `pwd`/hostfile-byol-test -f
cat $hostfile|sort|uniq >`pwd`/tmp

for i in `cat ./tmp`
do
    echo ${i} slots=4 >> `pwd`/hostfile-byol-test
done
...
mpirun -np $np --allow-run-as-root -hostfile `pwd`/hostfile-byol-test `pwd`/test_single_byol.sh $dist_url