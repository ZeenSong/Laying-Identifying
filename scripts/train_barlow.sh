...
#SBATCH --gres=dcu:4
#SBATCH --mem=100G
#SBATCH -o Train_Barlow.o
#SBATCH -e Train_Barlow.e
...
rm `pwd`/hostfile-barlow -f
cat $hostfile|sort|uniq >`pwd`/tmp

for i in `cat ./tmp`
do
    echo ${i} slots=4 >> `pwd`/hostfile-barlow
done
...
mpirun -np $num_DCU --allow-run-as-root -hostfile `pwd`/hostfile-barlow ./train_single_barlowtwins.sh $dist_url