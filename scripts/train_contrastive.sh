...
#SBATCH --gres=dcu:4
#SBATCH --mem=100G
#SBATCH -o Train_contrastive.o
#SBATCH -e Train_contrastive.e
...
rm `pwd`/hostfile-contrastive -f
cat $hostfile|sort|uniq >`pwd`/tmp

for i in `cat ./tmp`
do
    echo ${i} slots=4 >> `pwd`/hostfile-contrastive
done
...
mpirun -np $num_DCU --allow-run-as-root -hostfile `pwd`/hostfile-contrastive ./train_single_contrastive.sh $dist_url